...
def lfsr_initialize_mode(u):
    ...
    执行LFSR初始化模式。
    """
    v = (2 ** 15 * lfsr_state[15] + 2 ** 17 * lfsr_state[13] + 2 ** 21 * lfsr_state[10] +
...
def lfsr_operate_mode():
    ...
    执行LFSR工作模式。
    """
    lfsr_state.append((2 ** 15 * lfsr_state[15] + 2 ** 17 * lfsr_state[13] + 2 ** 21 * lfsr_state[10] +
                       2 ** 20 * lfsr_state[4] + (1 + 2 ** 8) * lfsr_state[0]) % (2 ** 31 - 1))
    if lfsr_state[16] == 0:
        lfsr_state[16] = 2 ** 31 - 1
    lfsr_state.pop(0)
...
def nonlinear_function(x0, x1, x2):
    ...
    temp2 = reg2 ^ x2
    reg1 = sbox_transform(l1_transform(((temp1 << 16) | (temp2 >> 16)) & 0xffffffff))
    reg2 = sbox_transform(l2_transform(((temp2 << 16) | (temp1 >> 16)) & 0xffffffff))
...
def l1_transform(x):
    ...
    执行线性变换L1。
    """
    return x ^ rotate_left(x, 2) ^ rotate_left(x, 10) ^ rotate_left(x, 18) ^ rotate_left(x, 24) & 0xffffffff
...
def l2_transform(x):
    ...
    执行线性变换L2。
    """
    return x ^ rotate_left(x, 8) ^ rotate_left(x, 14) ^ rotate_left(x, 22) ^ rotate_left(x, 30) & 0xffffffff
...
def encrypt_decrypt_text(text):
    ...
    encrypted_decrypted_bytes = bytes(
        [b ^ (keystream[i % keystream_length] & 0xff) for i, b in enumerate(text_bytes)])
